...
// (c) - dex / supremacy
// was too lazy to write my own vector3d class
...
public:
	...
	// at-accesors.
	__forceinline auto at( const size_t index ) -> float& {
		return reinterpret_cast<float*>(this)[index];
	}

	__forceinline auto at( const size_t index ) const -> float& {
		return ((float*)this)[index];
	}

	// index operators.
	__forceinline auto operator( )( const size_t index ) -> float& {
		return at(index);
	}

	__forceinline auto operator( )( const size_t index ) const -> const float& {
		return at(index);
	}
	...
	// equality operators.
	__forceinline auto operator==( const vec3_t& v ) const -> bool {
	...
	// copy assignment.
	__forceinline auto operator=( const vec3_t& v ) -> vec3_t& {
		x = v.x;
		y = v.y;
		z = v.z;
		return *this;
...
public:
	__forceinline vec_aligned_t( ): w(0) {
	}
