...
// (c) - dex / supremacy
// was too lazy to write my own vector2d class
...
public:
	...
	__forceinline vec2_t( const float x, const float y ) : x{x}, y{y} {
	}

	__forceinline vec2_t( int x, int y ) : x{static_cast<float>(x)}, y{static_cast<float>(y)} {
	}
